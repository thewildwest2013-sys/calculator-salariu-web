...
import { loginWithEmail, logoutUser, sendResetPasswordEmail } from "@/lib/auth";
...
export default function LoginPage() {
  ...
  async function handleForgotPassword() {
    ...
    try {
      ...
      const message = error instanceof Error ? error.message : "Nu am putut trimite emailul de resetare.";
      alert(message);
    } finally {
        ...
        {showResetBox && (
          <div className="mt-5 rounded-[20px] border border-white/10 bg-[#071326]/80 p-4">
            <p className="mb-4 text-sm text-white/70">
              Introdu emailul contului tău și îți trimitem un link pentru resetarea parolei.
            </p>
            ...
            >
              {resetLoading ? "Se trimite emailul..." : "Trimite email de resetare"}
            </button>
          </div>
        )}
