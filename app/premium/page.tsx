    ...
    <main className="app-shell p-6 md:p-8">
      <div className="mx-auto max-w-3xl rounded-[32px] border border-white/10 bg-white/[0.04] p-6 shadow-[0_0_60px_rgba(0,80,255,0.08)]">
        ...
        <div className="mt-6 rounded-[24px] border border-white/10 bg-[#071326]/80 p-4">
          ...
          <p className="mt-2"><strong>Status curent:</strong> {profile?.isPremium ? "PREMIUM" : "FREE"}</p>
          <p className="mt-2"><strong>Plan:</strong> {profile?.plan || "free"}</p>
        </div>
        ...
        <div className="mt-6 rounded-[24px] border border-white/10 bg-[#071326]/80 p-4">
          ...
          {!profile?.isPremium ? (
            <button onClick={activatePremium} disabled={checkoutLoading} className="primary-btn mt-6 w-full disabled:opacity-50">
            ...
            <div className="mt-6 rounded-[20px] border border-emerald-400/20 bg-emerald-400/10 p-4">
              ...
              <p className="mt-2 text-sm opacity-80">Abonamentul tău este activ și sincronizat.</p>
            </div>
          )}
        </div>
      </div>
