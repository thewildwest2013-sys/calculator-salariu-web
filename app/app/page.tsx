...
export default function Home() {
  ...
  const [activeTabBeforeGate, setActiveTabBeforeGate] = useState<TabKey>("rules");
  ...
  useEffect(() => {
    async function loadUsage() {
      ...
      try {
        ...
        console.error("Nu s-a putut încărca usage status:", error);
      } finally {
  ...
  async function refreshUsage() {
    ...
    try {
      ...
      console.error("Nu s-a putut reîmprospăta usage status:", error);
    }
  }
  ...
  async function unlockCalculationAndConsume() {
    ...
    try {
      ...
      if (!result.allowed) {
        ...
        alert(
          profile?.isPremium
            ? "Ai atins limita de 10 calcule în fereastra curentă de 24h. Revino după reset."
            : "Ai atins limita de 2 calcule în fereastra curentă de 24h. Activează Premium sau revino după reset.",
        );
        return;
      }
      ...
      console.error(error);
      alert("Nu s-a putut valida limita de utilizare.");
    } finally {
      setUsageLoading(false);
    }
  }
  ...
  async function handleCalculateRequest() {
    ...
    setActiveTabBeforeGate(activeTab);
    setAdGateOpen(true);
  }
          ...
          <div className="space-y-4">
            ...
            {adGateOpen && (
              <AdGateModal
                onClose={() => {
                  setAdGateOpen(false);
                  setActiveTab(activeTabBeforeGate);
                }}
...
function UsageCard({
  ...
  const limit = usageStatus?.limit ?? (isPremium ? 10 : 2);

  return (
    <div className="rounded-[28px] border border-white/10 bg-[#071326]/80 p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
      <div className="text-sm uppercase tracking-[0.18em] text-white/45">Utilizare</div>
      <div className="mt-3 text-2xl font-semibold md:text-3xl">
        {loading ? "Se verifică..." : `${remaining}/${limit}`}
      </div>
      <div className="mt-2 text-sm text-white/60">
        calcule rămase în fereastra de 24h
      </div>
    </div>
  );
}
...
function CalculationLockedSection({
  ...
  return (
    <SectionShell kicker="Calcul blocat" title="Deblochează estimarea">
      <div className="rounded-[28px] border border-white/10 bg-[#071326]/80 p-6">
        <p className="max-w-3xl text-lg leading-8 text-white/80">
          {isPremium
            ? "Pentru a genera o estimare nouă, folosește unul dintre calculele disponibile din fereastra curentă de 24h."
            : "Pe planul Free, fiecare calcul pornește după reclamă și este limitat la 2 calcule în 24h pe cont."}
        </p>

        <div className="mt-4 rounded-[22px] border border-white/10 bg-white/[0.03] px-4 py-3 text-sm text-white/70">
          Rămase acum: <span className="font-semibold text-white">{loading ? "Se verifică..." : usageStatus?.remaining ?? "—"}</span> / {usageStatus?.limit ?? (isPremium ? 10 : 2)}
        </div>
        ...
        >
          {isPremium ? "Generează estimarea" : "Vezi reclamă și generează estimarea"}
        </button>
      </div>
    </SectionShell>
  );
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/70 p-4 backdrop-blur-sm">
      <div className="w-full max-w-lg rounded-[32px] border border-white/10 bg-[#071326] p-6 shadow-[0_0_80px_rgba(0,80,255,0.18)]">
        ...
        <h3 className="mt-4 text-3xl font-bold">Deblochează calculul Free</h3>
        <p className="mt-3 text-base leading-7 text-white/75">
          Aici intră reclama fullscreen după ce îți configurezi formatul real de ads. Până atunci, fluxul de limită și deblocare rămâne pregătit în aplicație.
        </p>

        <div className="mt-4 rounded-[22px] border border-white/10 bg-white/[0.03] p-4 text-sm text-white/70">
          Calcule rămase în această fereastră de 24h: <span className="font-semibold text-white">{remaining}</span>
        </div>
    ...
    <SectionShell kicker="Setări personale" title="Reguli salariale">
      ...
      <div className="mt-5 flex flex-col gap-3 md:flex-row">
        <button
          ...
          className="rounded-[18px] bg-blue-600 px-6 py-3 text-base font-semibold transition hover:bg-blue-500 disabled:cursor-not-allowed disabled:opacity-60"
        >
          {props.isPremium ? "Calculează salariul" : "Vezi reclamă și calculează"}
        </button>
        <button
      ...
      <div className="mt-5 rounded-[20px] border border-white/10 bg-white/[0.03] p-4 text-sm leading-7 text-white/75">
        ...
        <p className="mt-2">
          {props.isPremium
            ? `Plan Premium: ai ${props.usageStatus?.remaining ?? "—"} calcule rămase în fereastra curentă de 24h (limită 10).`
            : `Plan Free: fiecare calcul pornește după reclamă și ai ${props.usageStatus?.remaining ?? "—"} calcule rămase în fereastra curentă de 24h (limită 2).`}
        </p>
      </div>
    </SectionShell>
  );
